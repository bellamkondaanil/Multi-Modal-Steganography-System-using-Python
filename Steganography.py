...
# ===================== IMAGE =====================
def encode_image(image_path, output_path, secret, key):
    if not os.path.exists(image_path):
        raise FileNotFoundError("Image not found")

    img = cv2.imread(image_path)
    secret = encrypt(secret, key) + DELIMITER
    binary_data = text_to_binary(secret)
...
# ===================== AUDIO =====================
def encode_audio(audio_path, output_path, secret):
    ...
    modified = bytes(frames)

    with wave.open(output_path, 'wb') as f:
        f.setparams(song.getparams())
        f.writeframes(modified)
...
# ===================== TEXT =====================
def encode_text(cover_file, output_file, secret):
    ...
    binary = text_to_binary(secret) + "111111111111"
    ZWC = {"00": '\u200C', "01": '\u202C', "11": '\u202D', "10": '\u200E'}
    ...
    for word in words:
        if idx < len(binary):
            chunk = binary[idx:idx+12]
            zwc = ''.join(ZWC[chunk[i:i+2]] for i in range(0, len(chunk), 2))
            result.append(word + zwc)
            idx += 12
        else:
            result.append(word)
...
def decode_text(stego_file):
    ...
    if "111111111111" in binary:
        binary = binary.split("111111111111")[0]
...
def main():
    key = generate_key()
    print("🔐 Your encryption key (SAVE THIS):", key.decode())

    while True:
        print("\n1. Image Encode")
        print("2. Image Decode")
        print("3. Audio Encode")
        print("4. Audio Decode")
        print("5. Text Encode")
        print("6. Text Decode")
        print("7. Exit")
        ...
        try:
            if choice == '1':
                encode_image(input("Image path: "), input("Output path: "), input("Message: "), key)
            elif choice == '2':
                print("Decoded:", decode_image(input("Image path: "), key))
            elif choice == '3':
                encode_audio(input("Audio path: "), input("Output path: "), input("Message: "))
            elif choice == '4':
                print("Decoded:", decode_audio(input("Audio path: ")))
            elif choice == '5':
                encode_text(input("Cover text file: "), input("Output file: "), input("Message: "))
            elif choice == '6':
                print("Decoded:", decode_text(input("Stego file: ")))
            elif choice == '7':
                break
